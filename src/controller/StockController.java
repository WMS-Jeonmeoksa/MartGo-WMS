...
public class StockController {
    ...
    public static void main(String[] args) {
        ...
        int incoming_num = 3;
        if (stockService.isIncomingStockUpdated(incoming_num)) {
            System.out.println("재고 업데이트가 성공적으로 완료되었습니다.");
        } else {
            System.out.println("재고 업데이트에 실패하였습니다.");
        }
        if (stockService.isIncomingStockHistoryUpdated(incoming_num)) {
        ...
        int outgoing_num = 1;
        if (stockService.isOutgoingStockUpdated(outgoing_num)){
            System.out.println("재고 이력 업데이트가 성공적으로 완료되었습니다.");
        } else{
            System.out.println("재고 이력 업데이트에 실패하였습니다.");
        }
        if (stockService.isOutgoingStockHistoryUpdated(outgoing_num)){
            System.out.println("재고 이력 업데이트가 성공적으로 완료되었습니다.");
        } else {
            System.out.println("재고 이력 업데이트에 실패하였습니다.");
        }
