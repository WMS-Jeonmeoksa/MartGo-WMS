...
public class ValidationUtil {
    ...
    // 2️⃣ 이름: 한글만 가능, 최소 2자 이상, 최대 10자
    public static boolean isValidName(String name) {
        return name != null && name.matches("^[가-힣]{2,10}$");
    }

    // 3️⃣ 비밀번호: 모든 문자(영어, 한글, 숫자 등) 가능, 최소 4자 이상
    public static boolean isValidPassword(String password) {
        return password != null && password.matches("^.{4,}$");
    }

    // 4️⃣ 전화번호: 10자리 또는 11자리 숫자만 가능 (하이픈 허용, 예: 0101234567, 01012345678, 010-1234-5678, 010-123-4567)
    public static boolean isValidPhone(String phone) {
        return phone != null && phone.matches("^01[0-9]-?(\\d{3,4})-?(\\d{4})$");
    }

    // 5️⃣ 이메일: 이메일 형식 (예: 2@2 도 허용)
